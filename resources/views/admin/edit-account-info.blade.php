    ...
    <main class="max-w-6xl mx-auto p-4 sm:p-6 lg:p-10">
      ...
      <header class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        ...
        <div class="flex items-center gap-3">
          <a href="{{ route('admin.employer.profile', $profile->id) }}"
      ...
      <section class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          <article x-data="{ status: '{{ $profile->account_status ?? 'Active' }}' }"
            class="bg-white p-6 rounded-lg shadow-sm">
            <h2 class="text-lg font-semibold mb-3">Account Info</h2>

            <form action="{{ route('admin.employer.account-info.update', $profile->id) }}" method="POST"
              class="space-y-6">
              @csrf
              @method('PUT')
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="text" readonly value="{{ $profile->last_login ?? 'N/A' }}"
                    class="mt-1 w-full bg-gray-50 border-gray-200 rounded-md text-sm font-medium text-gray-700 px-3 py-2">
                </div>
                ...
                <div>
                  ...
                  <select id="account_status" name="account_status" x-model="status"
                    class="px-3 py-2 mt-1 block w-full border-gray-300 rounded-md text-sm font-medium text-gray-700 focus:border-indigo-500 focus:ring-indigo-500">
                    <option value="Active" {{ $profile->account_status === 'Active' ? 'selected' : '' }}>Active</option>
                    <option value="Suspended" {{ $profile->account_status === 'Suspended' ? 'selected' : '' }}>Suspended
                    </option>
                    <option value="Pending Approval" {{ $profile->account_status === 'Pending Approval' ? 'selected' : '' }}>Pending Approval</option>
                  </select>
                </div>
